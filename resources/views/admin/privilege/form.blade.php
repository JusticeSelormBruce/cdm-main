                ...
                <form action="/admin/get-user-roles" method="post">
                    <div class="input-group-sm py-3">
                        ...
                        @else
                            <select name="user_id" required class="form-control" onchange="this.form.submit()">
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" @if($me==$user->id) selected @endif> {{ $user->name }} <span
                                            class="mx-2"></span>({{ $user->email }})</span>
                                    </option>
                                @endforeach
                            </select>
                                ...
                                <div class="row py-2">
                                    @foreach($privileges as $list)
                                        <div class="col-4 input-group-sm ">
                                            <input type="checkbox" name="role_id[]" value="{{ $list->id }}" class="mx-3 py-2">
                                            <span>{{ $list->name }}</span>
                                        </div>
                                    @endforeach
                            ...
                            <div class="design input-group-sm py-2">
                                ...
                                <div class="row py-2">
                                    @foreach($privileges as $list)
                                        <div class="col-4 input-group-sm ">
                                            <input type="checkbox" name="role_id[]" value="{{ $list->id }}" class="mx-3 py-2"
                                                @foreach($userRoles as $roles) @if($list->id ==$roles) checked @endif @endforeach>
                                            <span>{{ $list->name }}</span>
                                        </div>
                                    @endforeach
